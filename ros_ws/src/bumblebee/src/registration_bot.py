...
def callback1(data):
    global callback1_event
    global imgR
    with lock:
        imgR = bridge.imgmsg_to_cv2(data, "bgr8")
    callback1_event.set()  # Set the event to indicate callback1 has finished
    return imgR
...
def callback2(data):
    ...
    callback0_event.wait() # Wait for callback0 to finish
    callback1_event.wait() # Wait for callback1 to finish
    ...
    with lock:
        ...
        global disp_list
        global flag
        print("imgL shape from registration_bot: ", imgL.shape)
        print("imgR shape from registration_bot: ", imgR.shape)
    ...
    # callback0_event.clear()  # Reset the event for the next iteration
    # callback1_event.clear()  # Reset the event for the next iteration
...
if __name__ == '__main__':
    ...
    # Thread event to synchronize the callbacks
    callback0_event = threading.Event()
    callback1_event = threading.Event()
