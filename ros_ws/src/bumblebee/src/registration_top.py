...
###################################### Node to register point clouds from Top stereo (Cam 2 | Cam 3) images ######################################
...
def callback2(data):
    ...
    imgL = callback0(rospy.wait_for_message("/left_top_rect", Image))
    img_list_left.append(imgL)
    imgR = callback1(rospy.wait_for_message("/right_top_rect", Image))
    ...
    if (len(img_list_left)) == 7 and (len(img_list_right)) == 7 and (len(disp_list)) == 7:
        ...
        pcd_top_publisher.publish(pc2_msg)
        rate.sleep()
...
def listener():
    rospy.Subscriber("/left_top_rect", Image, callback0, queue_size=qs)
    rospy.Subscriber("/right_top_rect", Image, callback1, queue_size=qs)
    rospy.Subscriber("/disp_top", Float64MultiArray, callback2, queue_size=qs)
    rospy.spin()
...
if __name__ == '__main__':
    ...
    rospy.init_node('registration_top')
    pcd_top_publisher = rospy.Publisher("/pcd_top", PointCloud2, queue_size=qs)
